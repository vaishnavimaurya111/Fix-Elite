...
import { getAIEstimate } from "../../services/ai.service";
...
export default function ProblemDescription() {
  ...
  const analyzeWithAI = async () => {
    setIsAnalyzing(true);
    try {
      const response = await getAIEstimate(problemDescription, selectedService.name);
      
      if (response.success && response.data) {
        const aiResult: AIEstimate = response.data;
        setEstimate(aiResult);
        setAiEstimate(aiResult);
        setShowEstimate(true);
      } else {
        toast.error("AI analysis failed to generate a complete estimate.");
      }
    } catch (error) {
